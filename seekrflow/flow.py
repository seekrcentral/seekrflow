...
Manage all stages of seekrflow. This includes parameterization, preparation,
and running.
"""
...
def flow(
    ...
    """
    assert instruction in ["any", "parameterize", "prepare", "run"], \
        f"Invalid instruction '{instruction}'. Options are: 'any', "\
        "'parameterize', 'prepare', 'run'."
    doing_parameterize = False
    doing_prepare = False
    doing_run = False
    if instruction == "any":
        doing_parameterize = seekrflow.parameterizer is not None
        doing_prepare = True
        doing_run = True
    elif instruction == "parameterize":
        doing_parameterize = True
...
def main():
    argparser = argparse.ArgumentParser(
        description="Automates the parameterization, preparation, and running "
        "of seekr calculations for particular purposes, such as a "
    ...
    argparser.add_argument(
        ...
        "'any', which will automatically perform the next step, 'parameterize', "
        "which will parameterize the system, 'prepare', which will prepare the "
        "system, 'run', which will run the system.")
    argparser.add_argument(
    ...
    argparser.add_argument(
        ...
        help="Path to the work directory for the calculation.")
    argparser.add_argument(
